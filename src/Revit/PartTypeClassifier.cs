...
namespace PcfExport.Revit
{
    ...
    internal static class PartTypeClassifier
    {
        ...
        /// duct sections (rectangular / round / oval / flat-oval). PCF has
        /// no duct domain, so the geometric and PCF-name tests don't help
        /// here — this list is the fallback. Add additional CIDs as they
        /// are identified.
        /// </summary>
        ...
        public static string GetPcfType(FabricationPart part)
        {
            ...
            // Pass 1b — Product Range catalog classification. Runs
            // BEFORE the alias/description text scan because the fab
            // catalog's own "Joints" range is the most reliable
            // weld signal — catches every weld variant regardless
            // of how the family is named ("Weld Joint", "Field
            // Weld Joint", "Shop Weld Joint", "Butt Weld", "WLD",
            // whatever). Alias text-matching only catches specific
            // keyword patterns and misses catalog-specific
            // family names. Flanges/couplings/elbows live in
            // different Product Ranges so no false positive.
            if (IsJointByProductRange(part))
                return "WELD";
        ...
        /// </summary>
        /// <summary>True when the Fabrication part's catalog
        /// <c>Product Range</c> parameter contains "joint"
        /// (case-insensitive) — matches "Joints", "Pipe Joints",
        /// "Joint", etc. Product Range is catalog-defined, so it's
        /// stable across projects and doesn't depend on how a
        /// specific family got named. Flanges / couplings / elbows
        /// live in DIFFERENT Product Ranges so this is a safe weld
        /// signal.</summary>
        private static bool IsJointByProductRange(FabricationPart part)
        {
            if (part == null) return false;
            try
            {
                var p = part.LookupParameter("Product Range");
                if (p == null || p.StorageType != StorageType.String) return false;
                var range = p.AsString() ?? string.Empty;
